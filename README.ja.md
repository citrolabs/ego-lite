...
**AI エージェントが Web 自動化を最速で実行できるブラウザ**
...
ego (lite) は、あなたと AI エージェントが並行して作業できるブラウザです。エージェントがそれぞれの Space で複数のブラウザタスクを実行する一方で、あなたは自分の Space で自由にウェブを閲覧でき、エージェントがブラウザの操作権を奪うことはありません。さらに、ウェブの自動化タスクはより速く完了し、トークンの消費も少なくて済みます。

browser-use や agent-browser などの既存ツールは、ブラウザそのものではなく、ブラウザへの橋渡しです。自前のブラウザを備えているわけではありません。操作するには別のブラウザが必要で、ブラウザのデータをそのまま引き継ぐことはほとんどできず、接続も安定しません。最終的にはあなたとエージェントがブラウザの操作権を奪い合うことになります。一方、ego lite は、最初からあなたとエージェントの 2 者が共有するために設計された 1 つのブラウザです。追加のセットアップは不要で、エージェントは `ego-browser` を通じて、実際にログインしている状態やタブにいつでもアクセスできます。
...
ego lite は現在 macOS に対応しており、Windows 版はまもなくクローズドベータを開始する予定です。Linux は[ロードマップ](https://lite.ego.app/roadmap)にあります。
...
閲覧データや Cookie など、すべてのブラウザデータはお使いのデバイスに残ります。ego lite のデータ収集はごく限定的で、ego lite を既定のブラウザに設定したかどうかなど、簡単なプロダクトデータを確認するだけです。

## ego lite の特長

| 機能 | できること |
|---|---|
| **CLI 経由ではなくコードで直接操作。複雑なタスクをより速く、より少ないトークンで** | ego lite がエージェントに公開する機能は、エージェントが直接呼び出す JavaScript 関数としてラップされています。エージェントは得意なこと、つまりコードを書くことに専念できます。多段階のタスクを 1 回の出力にまとめられるので、「コマンドを 2 つ呼び出し、結果を見て、また 2 つ呼び出す」というループにはまりません。従来の CLI 方式と比べて、複雑なワークフローの実行が大幅に速くなり、タスク成功率も高く、タスクあたりのツール呼び出し回数もはるかに少なく、最終的にタスクにかかるコストも大きく削減されます。 |
...
| **同じブラウザ内の並列ワークスペース、Spaces でエージェントが同時に作業** | 各 Space にはそれぞれの AI エージェント、またはそれぞれのタスクが割り当てられ、すべてが同時に動きます。Claude Code が 10 件のリードを 10 個の並列 Space で強化する。Codex が 5 つの競合サイトをさらに 5 つの Space でスクレイピングする。互いに衝突することも、あなたのタブを奪うこともありません。マウスカーソルも置いた場所のままです。 |
...
| **どんなエージェントからでも `ego-browser` 経由で操作可能** | `ego-browser` は、あらゆるエージェント CLI（Claude Code、Codex、Cursor、あるいは自作のもの）と ego lite をつなぐ接続層です。ブラウザの機能を、snapshot・fill・click・wait・navigate・capture といったページ内 JavaScript ツール群として提供します。エージェントがこれらのツールを呼び出す JavaScript スニペットを書くと、`ego-browser` がそれをページ上で一度に実行します。 |
