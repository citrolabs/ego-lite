...
browser-use や agent-browser などの既存ツールは、本質的にはブラウザ自動化のブリッジツールです。操作するには別のブラウザが必要で、ブラウザのデータをそのまま引き継ぐことは難しく、接続も安定しません。最終的にはあなたとエージェントがブラウザの操作権を奪い合うことになります。一方、ego lite は最初からあなたとエージェントの 2 者が共有するために設計された 1 つのブラウザです。追加のセットアップは不要で、エージェントは `ego-browser` を通じて、実際にログインしている状態やタブにいつでもアクセスできます。
