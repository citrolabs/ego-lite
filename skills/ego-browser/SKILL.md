...
**Linux port caveat**: `browser.listTabs()` lists tabs across the whole browser, not just the selected task space, so filter by the space's `targetIds` when you need per-space tabs.
...
- `page.snapshot()` defaults to the whole page. Pass `{ scope: 'only_within_viewport' }` only when the task needs visible content alone.
