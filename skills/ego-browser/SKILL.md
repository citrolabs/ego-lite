...
**A task space can vanish or get reassigned without warning if another process shares the same browser instance** (e.g. a second concurrent ego-browser session) — a later `useOrCreateTaskSpace(id)` call throws `task space not found: <id>`. Treat this as a cold start, not an error to retry: call `useOrCreateTaskSpace(newName)` to open a fresh space and re-navigate to where you were; don't assume a previously located tab, search filter, or scroll position is still there. After taking over or recreating a space, re-verify current state with `snapshotText()` / `captureScreenshot()` before acting — don't act on remembered state from before the gap.
...
- **`captureScreenshot()` images can be scaled down from the real viewport** (e.g. a 2544×1303 page rendered as a 2000×1024 image) — clicking at coordinates measured directly off the image lands off-target. Check the image dimensions against `pageInfo()`'s `pw`/`ph` and multiply screenshot-measured coordinates by that ratio before calling `click([x, y])`. Prefer getting exact positions via `js()` with `getBoundingClientRect()` instead when possible — it returns real viewport coordinates directly and skips the conversion entirely.
