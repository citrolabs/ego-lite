...
- If `page.screenshot()` times out (`CDP request timed out: Page.captureScreenshot`) while `page.info()`, `page.evaluate()`, and navigation keep working, the ego lite window is most likely minimized or hidden: macOS stops issuing drawing signals for those states, so the capture never receives a frame. Ask the user to restore the window; it may stay in the background and be fully covered, it just must not be minimized or hidden. Retrying, a new tab, or a new task space does not clear it.
