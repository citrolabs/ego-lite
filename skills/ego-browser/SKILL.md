...
- `browser`: `listTabs`, `currentTab`, `switchTab`, `openOrReuseTab`, `closeTab`, `ensureRealTab`, `iframeTarget`. `closeTab` refuses to close the last remaining tab; destroy a space only through explicit `taskSpaces.complete(..., { keep: false })`.
