...
Always use `ego-cli` directly -- never `npx ego-cli`. The direct binary uses the fast Rust client. `npx` routes through Node.js and is significantly slower.
...
```bash
ego-cli --session {SESSION} open {TARGET_URL}
ego-cli --session {SESSION} wait --load networkidle
```
...
```bash
ego-cli --session {SESSION} snapshot -i
# Identify login form refs, fill credentials
ego-cli --session {SESSION} fill @e1 "{EMAIL}"
ego-cli --session {SESSION} fill @e2 "{PASSWORD}"
ego-cli --session {SESSION} click @e3
ego-cli --session {SESSION} wait --load networkidle
```
...
```bash
ego-cli --session {SESSION} state save {OUTPUT_DIR}/auth-state.json
```
...
```bash
ego-cli --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/initial.png
ego-cli --session {SESSION} snapshot -i
```
...
```bash
ego-cli --session {SESSION} snapshot -i
ego-cli --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/{page-name}.png
ego-cli --session {SESSION} errors
ego-cli --session {SESSION} console
```
...
```bash
ego-cli --session {SESSION} record start {OUTPUT_DIR}/videos/issue-{NNN}-repro.webm
```
...
```bash
ego-cli --session {SESSION} screenshot {OUTPUT_DIR}/screenshots/issue-{NNN}-step-1.png
sleep 1
# Perform action (click, fill, etc.)
sleep 1
ego-cli --session {SESSION} screenshot {OUTPUT_DIR}/screenshots/issue-{NNN}-step-2.png
sleep 1
...
```bash
sleep 2
ego-cli --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/issue-{NNN}-result.png
```

4. **Stop the video:**

```bash
ego-cli --session {SESSION} record stop
```
...
```bash
ego-cli --session {SESSION} screenshot --annotate {OUTPUT_DIR}/screenshots/issue-{NNN}.png
```
...
- **Be efficient with commands.** Batch multiple `ego-cli` commands in a single shell call when they are independent (e.g., `ego-cli ... screenshot ... && ego-cli ... console`). Use `ego-cli --session {SESSION} scroll down 300` for scrolling -- do not use `key` or `evaluate` to scroll.
