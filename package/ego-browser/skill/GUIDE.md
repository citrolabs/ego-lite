...
installed ego-browser Skill. Use `help()` for signatures and uncommon options of
APIs named below.
...
On Windows PowerShell or cmd, run `ego-browser skill windows` before the first
script.
...
IIFE, open a new task space, or resend the same command. Write the script to a
UTF-8 `.mjs` file and import it instead; `ego-browser skill windows` shows that
form.
...
FileChooser, mouse, and keyboard APIs explicitly listed in this guide. Do not
...
`ego-browser upgrade` only with their approval. Run `ego-browser skill` again
after the upgrade.
