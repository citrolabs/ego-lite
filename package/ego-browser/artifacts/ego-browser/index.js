...
function serverInfoPath(taskId) {
...
async function readStoredTask(taskId) {
    return readJson(join(taskDir(taskId), "studio.json"));
}
async function writeStoredTask(spec, existing) {
...
function documentToA2UI(spec, surfaceId) {
    const components = [];
...
async function readServerInfo(taskId) {
    const path = serverInfoPath(taskId);
    if (!existsSync(path))
        return null;
    return readJson(path);
}
async function writeServerInfo(taskId, info) {
 ...
 */
async function createWorkStudio(spec) {
...
async function readExistingTask(taskId) {
    try {
...
async function ensureServer(task) {
...
async function serverAlive(host, port, task) {
    try {
...
async function applyTabPolicy(task) {
    const policy = {
...
var helpers = /*#__PURE__*/Object.freeze({
    ...
    createWorkStudio: createWorkStudio,
    currentTab: currentTab,
    dispatchKey: dispatchKey,
    doubleClick: doubleClick,
    dragMouse: dragMouse,
...
async function startWorkStudioServer(options) {
...
async function handleRequest(taskId, req, res) {
...
function authorized(task, req, url) {
...
async function restoreHistory(task, body) {
...
async function openOriginalItem(task, itemId) {
    const item = task.items.find((candidate) => candidate.id === itemId);
    if (!item)
        return { ok: false, status: "not_found", message: `item not found: ${itemId}` };
    const source = sourceTab(task, item.sourceTabId, item.url);
    if (source?.targetId) {
    ...
    if (!url)
        return { ok: false, status: "missing_url", message: "item has no URL" };
...
async function checkoutItem(task, itemId, body) {
    const item = task.items.find((candidate) => candidate.id === itemId);
    if (!item)
        return { ok: false, status: "not_found", message: `item not found: ${itemId}` };
    ...
    const source = sourceTab(task, item.sourceTabId, item.url);
    const url = checkoutUrl(item) || item.url || source?.url;
    if (!url) {
        return {
            ok: false,
...
async function runAction(task, actionName, body) {
...
async function runBinding(task, bindingId, body) {
    const binding = task.bindings.find((candidate) => candidate.id === bindingId);
    if (!binding)
        return { ok: false, status: "not_found", message: `binding not found: ${bindingId}` };
    const item = binding.itemId ? task.items.find((candidate) => candidate.id === binding.itemId) : null;
    const source = sourceTab(task, binding.sourceTabId || item?.sourceTabId, item?.url);
    if (source?.targetId) {
        await switchTab(source.targetId);
    }
...
function isCheckoutBinding(binding) {
...
function sourceTab(task, sourceTabId, fallbackUrl) {
    const tab = sourceTabId ? task.tabs.find((candidate) => candidate.id === sourceTabId) : null;
    if (tab)
        return tab;
    if (fallbackUrl)
        return task.tabs.find((candidate) => candidate.url === fallbackUrl) || { url: fallbackUrl };
    return null;
}
function historyA2UI(task) {
...
function statusA2UI(task, bindingId, message) {
    return [{
            version: "v0.9",
            updateDataModel: {
...
async function appendAudit(task, event) {
    const audit = [...(task.audit || []), { ...event, at: new Date().toISOString() }];
    await writeStoredTask({ ...task, data: { ...(task.data || {}), audit } }, task);
}
function shellHtml(task) {
...
async function runWorkStudioServer(taskId) {
    if (!taskId) {
